...
int main(int argc, char** argv) {
  ...
  ll_init();
	...
	printf("\tget_name: %s\n", ll_get_name("http://creativecommons.org/licenses/by-nd/2.0/"));
	printf("\tget_version: ");
	...
	char** result = ll_get_attribute("http://creativecommons.org/licenses/by-nd/2.0/za/","http://purl.org/dc/elements/1.1/title");
	int i =0;
	while(result[i]!=NULL)
		printf("\t\t%s\n",result[i++]);
