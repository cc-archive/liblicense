...
int ll_set_default(const ll_uri_t u) {
	...
	while(modules[i]!=NULL) {
		...
		set_default = ll_get_module_symbol(LIBLICENSE_CONFIG_MODULE_DIR,modules[i],"set_default");
...
ll_uri_t ll_get_default() {
	...
	while(modules[i]!=NULL) {
		...
		ll_module_init(LIBLICENSE_CONFIG_MODULE_DIR,modules[i]);
		get_default = ll_get_module_symbol(LIBLICENSE_CONFIG_MODULE_DIR,modules[i],"get_default");
