...
struct _ll_license_chooser_t
{
  ...
  /**
   * This is the meat of the license chooser.  It is an array
   * of linked-lists of licenses, where each slot holds all the
   * licenses which match a given combination of license flags.
   * The array holds all possible combinations of attributes.
   *
   * Finding the matching index based on a set of flags involves
   * traversing this structure in a tree-like manner.  A
   * tree built for searching based on the Attribution and
   * CommercialUse attributes is structured like this example:
   *
   *                                             root
   *                        /               |            |              \
   ...
   *
   * In the example above, the list of licenses that require (R)
   * Attribution and prohibit (Pr) CommercialUse is found at
   * license_list[6].
   *
   * Such a structure allows easy hierarchical traversion.  Ignore
   * the indices above and instead index the tree as a d-heap
   * (where d=N_STATES). A node's left-most child is always
   * (node_index-N_STATES+2)*N_STATES.  N_STATES is 4 for P, R Pr, and U.
   */
  ll_license_list_t **license_list;

  /**
   * to avoid copying each license, store the license entire
   * license list and free them all in one go when we destroy the
   * license_chooser
   */
  ll_uri_t *all_licenses;
};
...
/**
 * Returns the total number of nodes in the heap built for searching on
...
/*
 * Recursively traverse the license heap, increasing the appropriate
 * index in license_hits.  When the traversal is complete, the license
 * which has <num_attribute> hits is the matched license.
 *
 * For example, one call of iterate_children will increase
 * license_hits[] for all licenses that prohibit CommercialUse.  On the
 * next call to iterate_children, all licenses that require Attribution
 * are increased.  Using the above tree in this example, first,
 * license_hits[2,6,10,14] are all increased to 1.  Then the second call
 * increases license_hits[4,5,6,7]. license_hits[6] is now 2, meaning
 * we've matched the licenses at that index.
 */
static void iterate_children( int *license_hits, int idx, int height,
                              int heap_sz )
{
        int leftChild;
        int i;
...
static int attribute_index( const char **attributes, char *attr,
                            int num_attributes )
{
        int i;
...
const ll_license_list_t* ll_get_licenses_from_flags( ll_license_chooser_t *license_chooser, int permits, int requires, int prohibits )
{
         ...
         * Each pass in the for-loop traverses down one level of the
         * tree.
         */
        int curr = N_STATES - 1;
        int i;
